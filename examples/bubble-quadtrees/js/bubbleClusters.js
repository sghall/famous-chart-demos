define(function(require, exports, module) {
  ...
  var updateView = function (size, varname) {
    ...
    force.on('tick', tickFactory(centers, varname, .55));
    force.start();
    labels(centers);
  }

  var tickFactory = function (centers, varname, k) {
    var duration = 8000, foci = {};
    var start = Date.now();
    for (var i = 0; i < centers.length; i++) {
      foci[centers[i].name] = centers[i];
    }
    return function (e) {
      var target, actual, telapsed, quad;
      for (var d = 0, len = data.length; d < len; d++) {
        actual = data[d];
        target = foci[actual[varname]];
        actual.y += ((target.y + target.dy / 2) - actual.y) * k * e.alpha;
        actual.x += ((target.x + target.dx / 2) - actual.x) * k * e.alpha;
      }
      quad = collide(.1);
      for (var i = 0, len = data.length; i < len; i++) {
        ...
        // }
        data[i].modifier.setTransform(
          Transform.translate(data[i].x, data[i].y, 1)
        );
      }
    }
  }
